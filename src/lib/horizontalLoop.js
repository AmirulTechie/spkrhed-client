...
// Adapted from GSAP's official "horizontalLoop" helper function
// (https://gsap.com/docs/v3/HelperFunctions/helpers/seamlessLoop). Builds a
// timeline that seamlessly loops a row of same-height-container items
// horizontally and, when config.draggable is true, wires up Draggable +
// InertiaPlugin so the loop can be scrubbed by grabbing any item — with
// momentum on release and a snap to the nearest item.
export function horizontalLoop(items, config) {
  ...
  const ctx = gsap.context(() => {
    ...
    const populateTimeline = () => {
      let item, curX, distanceToStart, distanceToLoop;
      tl.clear();
    ...
    if (config.draggable && typeof Draggable === "function") {
      ...
      let ratio, startProgress, draggable, lastSnap, initChangeX;
