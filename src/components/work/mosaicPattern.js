...
// columns has its own independent sequence of tile heights (in whole
// grid-cell units), so row seams don't line up across columns — a tile's
// bottom edge in column 0 lands at a different y than its neighbor in
// column 1, the way a real masonry/Pinterest layout staggers.
//
// Every column's sequence sums to the same BLOCK_ROWS (6 units), so each
// column still tiles seamlessly with itself when repeated vertically, and
// all 4 columns together still form one clean BLOCK_COLS x BLOCK_ROWS
// rectangle that WorkGrid can repeat edge-to-edge for the infinite pan.
...
// row/rowSpan in whole grid-cell units, project cycling through
// WORK_PROJECTS across all 4 columns (12 slots total, 12 < 26 so nothing
// repeats within a single block).
